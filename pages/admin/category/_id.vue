<template>
  <div class="full-page-takeover-window">
    <div class="full-page-takeover-page">
      <Header
        :title="`Step ${activeStep} of ${stepsLength} to Update Category`"
...
export default {
  ...
  async asyncData({ $axios, params }) {
    try {
      ...
      const [categoryResponse] = await Promise.all([category]);
      ...
      this.$store
        ...
        .then((_) =>
          this.$toasted.success(
            `The category <strong class="has-text-white" style="margin: 0 10px; display: inline-block;"> ${this.form.title} </strong> has been updated successfully..`,
            { duration: 3500 }
          )
        );
